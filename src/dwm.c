		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				...
				for (bar = m->bar; bar; bar = bar->next)
					XMoveResizeWindow(dpy, bar->win, bar->bx, bar->by, bar->bw,
									  bar->bh);
			}
			focus(NULL);
			arrange(NULL);
...
void drawbarwin(Bar *bar) {
	...
	int rx, lx, rw, lw; // bar size, split between left and right if a center
						// module is added
	const BarRule *br;
	...
	if (bar->borderpx) {
		...
		XFillRectangle(drw->dpy, drw->drawable, drw->gc, 0, 0, bar->bw,
					   bar->bh);
	}
	...
	for (r = 0; r < LENGTH(barrules); r++) {
		...
		if (lw <= 0) { // if left is exhausted then switch to right side, and
					   // vice versa
			lw = rw;
			lx = rx;
		} else if (rw <= 0) {
...
void restack(Monitor *m) {
	...
	if (m->lt[m->sellt]->arrange) {
		...
		if (m->bar) {
			...
			for (f = m->stack; f && (f->isfloating || !ISVISIBLE(f));
				 f = f->snext)
...
void updatebars(void) {
	...
	for (m = mons; m; m = m->next) {
		for (bar = m->bar; bar; bar = bar->next) {
			...
			if (!bar->win) {
				bar->win =
					XCreateWindow(dpy, root, bar->bx, bar->by, bar->bw, bar->bh,
		...
		if (c->isurgent) {
			if (c->isfloating)
				XSetWindowBorder(dpy, c->win,
								 scheme[SchemeUrg][ColFloat].pixel);
			else
				XSetWindowBorder(dpy, c->win,
